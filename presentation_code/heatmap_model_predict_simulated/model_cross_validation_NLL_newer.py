...
""" NEWER VERSION:
    Implemented cross-fold validation (remember to set N). Converted finding NLLs into a function. 
    Sum NLLs across the cross-validation. This script works for distances. Can use another for head angle """
...
EXAMPLE_NLLs_DIST = [209.3421981,  244.38414186, 309.07730689, 282.09366861]
EXAMPLE_NLLs_HEADANG = [210.88248726, 256.88697357, 299.59442668, 305.00875267]
...
# function to find the negative log likelihoods for each type of model (specified within) given a training and test set
# purely for convenience, not intended for general use
def find_NLL(idxs_training, idxs_test):
    ...
    ### CHANGE HERE BETWEEN HEAD ANGLES AND DISTANCES

    # # DISTANCE-CENTRIC MODELS
    #   # dropped: head_angles_wall_1, head_angles_wall_2
    ##          all besides distances_wall_1
    ##          all besides distances_wall_2
    # dropped_models_idxs = [12, 10, 6] 

    # HEAD ANGLE-CENTRIC MODELS
    ## dropped: distances_wall_1, distances_wall_2
    ##          all besides head_angles_wall_1
    ##          all besides head_angles_wall_2
    dropped_models_idxs = [1, 5, 4]
    ...
    return full_model, dropped_models, dropped_models_desc, NLLs, mean_diffs
...
# curtail dataset so it is divisible by n (n-fold cross-validation)
data_size = idx_array.size
multiples_of_n = int(np.floor(data_size/N))
idx_array = idx_array[:multiples_of_n*N]
df = df[:idx_array.size]

# split data into N random and equal sets
idxs_randomised = np.random.choice(idx_array, int(idx_array.size), replace=False)
idxs_test_list = np.split(idxs_randomised, N, axis=0)

# find the training set for each of these N test sets
idxs_training_list = []
for test_set in idxs_test_list:
    idxs_training_list.append(np.array([i for i in range(df.shape[0]) if i not in test_set]))

## In future, for speed, use code like this instead of a list comp:
...
cross_val_NLLs = []
for i in range(N):
    cross_val_NLLs.append(find_NLL(idxs_training_list[i], idxs_test_list[i])[3])

stacked_NLLs = np.vstack(cross_val_NLLs)
summed_NLLs = np.sum(stacked_NLLs, axis=0)
print(summed_NLLs)
