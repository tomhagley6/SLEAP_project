...
""" functions to:
    ...
    - predict on simulated data and visualise """


def train_and_predict(data, xvars, yvar, drop_vars, predict_vars):
    ...
    dropped_models.append(model_drop)
    dropped_model_info.append("Dropped distances_wall_1 and distances_wall_2 from full model")

    print(f"LRT statistic: {LRT:.4f}")
    print(f"P-value: {p_value:.4f}\n")

    return log_reg, dropped_models, dropped_model_info
...
data_input = {'chose_high':chose_high.astype('int'), 'bias_term':bias_term, 
        ...
        'head_to_wall_1_angs':df.head_angles_wall_1.values, 'head_to_wall_2_angs':df.head_angles_wall_2.values}
...
# execute train
(full_model, dropped_model, 
dropped_model_descriptions) = train_and_predict(data_input, var_names, 'chose_high',
                                                 var_names, ['bias_term', 'head_to_wall_1_angs', 'head_to_wall_2_angs'])

# execute predict
predict_and_visualise(sim_data, full_model, ['bias_term', 'head_angle_wall_1', 'head_angle_wall_2'], 2.01)