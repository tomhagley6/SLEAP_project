...
def plot_start_points(session_trajectories, node, title=''):
    ...
    # append the starting coordinate of each trial to a list, then plot these
    for i in range(session_trajectories.shape[0]):
        start_x.append(session_trajectories[f"{node}_x"].iloc[i][0])
        start_y.append(session_trajectories[f"{node}_y"].iloc[i][0])
    ...
    plt.scatter(start_x, start_y, s=10, cmap=cmap, alpha=0.4)
    plt.axis('scaled')
    plt.xlim([-1.15, 1.15])
    plt.ylim([-1.15, 1.15])
