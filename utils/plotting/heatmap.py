...
def heatmap(x, y, val, nbins, bin_max, bin_min=0, v_max=1, v_min=0, cmap='crest', x_title=None, y_title=None, cut_low_n=True):
    ...
    fig, p1 = plt.subplots()
    p1 = sns.heatmap(heatmap_bin_means, vmin=v_min, vmax=v_max, cmap='rocket', 
                     xticklabels=bin_centres, yticklabels=bin_centres,mask=np.isnan(heatmap_bin_means))
    # the below will set x_ticks to be in the center of boxes, and to be the chose values
    # p1.set_xticks([0,1,2,3,4,5])
    # p1.set_xticklabels([0,36, 72, 108, 144, 180])
